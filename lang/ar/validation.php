<?php

return [
    'accepted' => 'حقل :attribute يجب أن يكون مقبولًا.',
    'accepted_if' => 'حقل :attribute يجب أن يكون مقبولًا عندما يكون :other هو :value.',
    'active_url' => 'حقل :attribute يجب أن يكون عنوان URL صالحًا.',
    'after' => 'حقل :attribute يجب أن يكون تاريخًا بعد :date.',
    'after_or_equal' => 'حقل :attribute يجب أن يكون تاريخًا بعد أو يساوي :date.',
    'alpha' => 'حقل :attribute يجب أن يحتوي على أحرف فقط.',
    'alpha_dash' => 'حقل :attribute يجب أن يحتوي على أحرف وأرقام وعلامات ناقصة وشرطة تحتية فقط.',
    'alpha_num' => 'حقل :attribute يجب أن يحتوي على أحرف وأرقام فقط.',
    'array' => 'حقل :attribute يجب أن يكون مصفوفة.',
    'before' => 'حقل :attribute يجب أن يكون تاريخًا قبل :date.',
    'before_or_equal' => 'حقل :attribute يجب أن يكون تاريخًا قبل أو يساوي :date.',
    'between' => [
        'array' => 'حقل :attribute يجب أن يحتوي على :min و :max عناصر.',
        'file' => 'حقل :attribute يجب أن يكون بين :min و :max كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون بين :min و :max.',
        'string' => 'حقل :attribute يجب أن يكون بين :min و :max حرفًا.',
    ],
    'boolean' => 'حقل :attribute يجب أن يكون صحيحًا أو خطأ.',
    'can' => 'حقل :attribute يحتوي على قيمة غير مسموح بها.',
    'confirmed' => 'حقل :attribute لا يتطابق مع التأكيد.',
    'current_password' => 'كلمة المرور غير صحيحة.',
    'date' => 'حقل :attribute يجب أن يكون تاريخًا صالحًا.',
    'date_equals' => 'حقل :attribute يجب أن يكون تاريخًا مساويًا لـ :date.',
    'date_format' => 'حقل :attribute يجب أن يتطابق مع الشكل :format.',
    'decimal' => 'حقل :attribute يجب أن يحتوي على :decimal أماكن عشرية.',
    'declined' => 'حقل :attribute يجب أن يكون مرفوضًا.',
    'declined_if' => 'حقل :attribute يجب أن يكون مرفوضًا عندما يكون :other هو :value.',
    'different' => 'حقل :attribute و :other يجب أن يكونوا مختلفين.',
    'digits' => 'حقل :attribute يجب أن يكون :digits أرقام.',
    'digits_between' => 'حقل :attribute يجب أن يكون بين :min و :max أرقام.',
    'dimensions' => 'حقل :attribute يحتوي على أبعاد صورة غير صالحة.',
    'distinct' => 'حقل :attribute يحتوي على قيمة مكررة.',
    'doesnt_end_with' => 'حقل :attribute يجب ألا ينتهي بأحد القيم التالية: :values.',
    'doesnt_start_with' => 'حقل :attribute يجب ألا يبدأ بأحد القيم التالية: :values.',
    'email' => 'حقل :attribute يجب أن يكون عنوان بريد إلكتروني صالح.',
    'ends_with' => 'حقل :attribute يجب أن ينتهي بأحد القيم التالية: :values.',
    'enum' => ':attribute المحدد غير صالح.',
    'exists' => ':attribute المحدد غير صالح.',
    'extensions' => 'حقل :attribute يجب أن يكون لديه إمتداد واحد من الإمتدادات التالية: :values.',
    'file' => 'حقل :attribute يجب أن يكون ملفًا.',
    'filled' => 'حقل :attribute يجب أن يحتوي على قيمة.',
    'gt' => [
        'array' => 'حقل :attribute يجب أن يحتوي على أكثر من :value عناصر.',
        'file' => 'حقل :attribute يجب أن يكون أكبر من :value كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون أكبر من :value.',
        'string' => 'حقل :attribute يجب أن يكون أكبر من :value حرفًا.',
    ],
    'gte' => [
        'array' => 'حقل :attribute يجب أن يحتوي على :value عنصر أو أكثر.',
        'file' => 'حقل :attribute يجب أن يكون أكبر من أو يساوي :value كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون أكبر من أو يساوي :value.',
        'string' => 'حقل :attribute يجب أن يكون أكبر من أو يساوي :value حرفًا.',
    ],
    'hex_color' => 'حقل :attribute يجب أن يكون لونا هكساديسيمال صالحًا.',
    'image' => 'حقل :attribute يجب أن يكون صورة.',
    'in' => ':attribute المحدد غير صالح.',
    'in_array' => 'حقل :attribute لا يوجد في :other.',
    'integer' => 'حقل :attribute يجب أن يكون عددًا صحيحًا.',
    'ip' => 'حقل :attribute يجب أن يكون عنوان IP صالحًا.',
    'ipv4' => 'حقل :attribute يجب أن يكون عنوان IPv4 صالحًا.',
    'ipv6' => 'حقل :attribute يجب أن يكون عنوان IPv6 صالحًا.',
    'json' => 'حقل :attribute يجب أن يكون سلسلة JSON صالحة.',
    'lowercase' => 'حقل :attribute يجب أن يكون حروف صغيرة.',
    'lt' => [
        'array' => 'حقل :attribute يجب أن يحتوي على أقل من :value عنصر.',
        'file' => 'حقل :attribute يجب أن يكون أقل من :value كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون أقل من :value.',
        'string' => 'حقل :attribute يجب أن يكون أقل من :value حرفًا.',
    ],
    'lte' => [
        'array' => 'حقل :attribute يجب ألا يحتوي على أكثر من :value عنصر.',
        'file' => 'حقل :attribute يجب أن يكون أقل من أو يساوي :value كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون أقل من أو يساوي :value.',
        'string' => 'حقل :attribute يجب أن يكون أقل من أو يساوي :value حرفًا.',
    ],
    'mac_address' => 'حقل :attribute يجب أن يكون عنوان MAC صالحًا.',
    'max' => [
        'array' => 'حقل :attribute يجب ألا يحتوي على أكثر من :max عنصر.',
        'file' => 'حقل :attribute يجب ألا يكون أكبر من :max كيلو بايت.',
        'numeric' => 'حقل :attribute يجب ألا يكون أكبر من :max.',
        'string' => 'حقل :attribute يجب ألا يكون أكبر من :max حرفًا.',
    ],
    'max_digits' => 'حقل :attribute يجب ألا يحتوي على أكثر من :max أرقام.',
    'mimes' => 'حقل :attribute يجب أن يكون ملف من النوع: :values.',
    'mimetypes' => 'حقل :attribute يجب أن يكون ملف من النوع: :values.',
    'min' => [
        'array' => 'حقل :attribute يجب أن يحتوي على الأقل على :min عنصر.',
        'file' => 'حقل :attribute يجب أن يكون على الأقل :min كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون على الأقل :min.',
        'string' => 'حقل :attribute يجب أن يكون على الأقل :min حرفًا.',
    ],
    'min_digits' => 'حقل :attribute يجب أن يحتوي على الأقل على :min أرقام.',
    'missing' => 'حقل :attribute يجب أن يكون مفقودًا.',
    'missing_if' => 'حقل :attribute يجب أن يكون مفقودًا عندما يكون :other هو :value.',
    'missing_unless' => 'حقل :attribute يجب أن يكون مفقودًا إلا إذا كان :other هو :value.',
    'missing_with' => 'حقل :attribute يجب أن يكون مفقودًا عندما يكون :values موجودًا.',
    'missing_with_all' => 'حقل :attribute يجب أن يكون مفقودًا عندما يكون :values موجودًا.',
    'multiple_of' => 'حقل :attribute يجب أن يكون ضعفًا لـ :value.',
    'not_in' => ':attribute المحدد غير صالح.',
    'not_regex' => 'شكل حقل :attribute غير صالح.',
    'numeric' => 'حقل :attribute يجب أن يكون رقمًا.',
    'password' => [
        'letters' => 'حقل :attribute يجب أن يحتوي على حرف واحد على الأقل.',
        'mixed' => 'حقل :attribute يجب أن يحتوي على حرف واحد كبير وحرف واحد صغير على الأقل.',
        'numbers' => 'حقل :attribute يجب أن يحتوي على رقم واحد على الأقل.',
        'symbols' => 'حقل :attribute يجب أن يحتوي على رمز واحد على الأقل.',
        'uncompromised' => ':attribute المعطى قد ظهر في تسريب بيانات. يرجى اختيار :attribute مختلفًا.',
    ],
    'present' => 'حقل :attribute يجب أن يكون موجودًا.',
    'present_if' => 'حقل :attribute يجب أن يكون موجودًا عندما يكون :other هو :value.',
    'present_unless' => 'حقل :attribute يجب أن يكون موجودًا إلا إذا كان :other هو :value.',
    'present_with' => 'حقل :attribute يجب أن يكون موجودًا عندما يكون :values موجودًا.',
    'present_with_all' => 'حقل :attribute يجب أن يكون موجودًا عندما يكون :values موجودًا.',
    'prohibited' => 'حقل :attribute ممنوع.',
    'prohibited_if' => 'حقل :attribute ممنوع عندما يكون :other هو :value.',
    'prohibited_unless' => 'حقل :attribute ممنوع إلا إذا كان :other هو :value.',
    'prohibits' => 'حقل :attribute يمنع :other من الوجود.',
    'regex' => 'شكل حقل :attribute غير صالح.',
    'required' => 'حقل :attribute مطلوب.',
    'required_array_keys' => 'حقل :attribute يجب أن يحتوي على مفاتيح الإدخال التالية: :values.',
    'required_if' => 'حقل :attribute مطلوب عندما يكون :other هو :value.',
    'required_if_accepted' => 'حقل :attribute مطلوب عندما يكون :other مقبولًا.',
    'required_unless' => 'حقل :attribute مطلوب ما لم يكن :other هو :values.',
    'required_with' => 'حقل :attribute مطلوب عندما يكون :values موجودًا.',
    'required_with_all' => 'حقل :attribute مطلوب عندما يكون :values موجودًا.',
    'required_without' => 'حقل :attribute مطلوب عندما لا تكون :values موجودًا.',
    'required_without_all' => 'حقل :attribute مطلوب عندما لا تكون أي من :values موجودة.',
    'same' => 'حقل :attribute و :other يجب أن يتطابقا.',
    'size' => [
        'array' => 'حقل :attribute يجب أن يحتوي على :size عنصر.',
        'file' => 'حقل :attribute يجب أن يكون :size كيلو بايت.',
        'numeric' => 'حقل :attribute يجب أن يكون :size.',
        'string' => 'حقل :attribute يجب أن يكون :size حرفًا.',
    ],
    'starts_with' => 'حقل :attribute يجب أن يبدأ بأحد القيم التالية: :values.',
    'string' => 'حقل :attribute يجب أن يكون سلسلة.',
    'timezone' => 'حقل :attribute يجب أن يكون منطقة صالحة.',
    'unique' => ':attribute مأخوذ بالفعل.',
    'uploaded' => 'فشل حقل :attribute في التحميل.',
    'uppercase' => 'حقل :attribute يجب أن يكون حروف كبيرة.',
    'url' => 'شكل حقل :attribute غير صالح.',
    'ulid' => 'حقل :attribute يجب أن يكون ULID صالحًا.',
    'uuid' => 'حقل :attribute يجب أن يكون UUID صالحًا.',

    'custom' => [
        'attribute-name' => [
            'rule-name' => 'custom-message',
        ],
    ],

    'attributes' => [
        'entry_time' => "وقت الدخول",
        'exit_time' => "وقت الخروج",
        'twelve_hours_price' => 'اجمالى سعر 12 ساعة',
        'hours_price' => 'اجمالى سعر 6 ساعة',
        'name' => 'الإسم',
        'username' => 'اسم المُستخدم',
        'email' => 'البريد الالكتروني',
        'first_name' => 'الإسم الأول',
        'last_name' => 'اسم العائلة',
        'password' => 'كلمة المرور',
        'password_confirmation' => 'تأكيد كلمة المرور',
        'city' => 'المدينة',
        'country' => 'الدولة',
        'phone' => 'الهاتف',
        'mobile' => 'الجوال',
        'age' => 'العمر',
        'sex' => 'الجنس',
        'gender' => 'النوع',
        'day' => 'اليوم',
        'month' => 'الشهر',
        'year' => 'السنة',
        'hour' => 'ساعة',
        'minute' => 'دقيقة',
        'second' => 'ثانية',
        'title' => 'العنوان',
        'content' => 'المُحتوى',
        'description' => 'الوصف',
        'excerpt' => 'المُلخص',
        'date' => 'التاريخ',
        'time' => 'الوقت',
        'available' => 'مُتاح',
        'size' => 'الحجم',
        'image' => 'الصورة الشخصية',
        'office_name' => 'إسم المكتب',
        'phone' => 'رقم الهاتف',
        //'description' => 'الوصف',
        'featured_descrip' => '(أكتر ما يميز العقار)',
        'detailed_address' => '(العنوان بالتفصيل)',
        'city_id'       => 'المدينة',
        'district_id'   => 'المنطقة',
        'lat'           => 'خط العرض',
        'lng'           => 'خط الطول',
        'group'         => 'النوع',
        'address'       => 'العنوان',
        'user_address'       => 'العنوان',
        'new_phone'     => 'رقم الهاتف',
        'residency_number_1' => "رقم الهوية",
        'residency_number_2' => "رقم الاقامة",
        'residency_number_3' => "رقم جواز السفر",
        'birthdate' => 'تاريخ الميلاد',
        "daily_price" => "السعر اليومى",
        "space" => 'المساحه',
        "hours price" => "سعر الساعه",
        "hours_no" => "عدد الساعات",
        'user_id' => 'المستخدم',
        'cancellation_tax_type' => 'نوع ضريبة الالغاء',
        'cancellation_tax' => 'خصم مبلغ عند إلغاء الحجز',
        'gift_name' => 'اسم الصديق',
        'gift_phone' => 'هاتف الصديق',
        "deposit_value" => "قيمة العربون",
        "cancellation_tax_value" => "قيمة الخصم",
        'payment_way_value' => 'المبلغ',
        'plate_number' => 'رقم اللوحة',
        'vehicle_name' => 'اسم السيارة',
        'color' => 'اللون',
        'vehicle_model_id' => 'موديل السيارة',
        'vehicle_type_id' => 'نوع السيارة',
        'image' => 'الصورة',
        'license_front_image' => 'صورة الرخصة الأمامية',
        'license_back_image' => 'صورة الرخصة الخلفية',
        'country_code' => 'كود الدولة',
        'phone' => 'رقم الهاتف',
        'email' => 'البريد الإلكتروني',
        'password' => 'كلمة المرور',
        'password_confirmation' => 'تأكيد كلمة المرور',
        'old_password' => 'كلمة المرور القديمة',
        'new_password' => 'كلمة المرور الجديدة',
        'new_password_confirmation' => 'تأكيد كلمة المرور الجديدة',
        'vehicle_id' => 'السيارة',
        'otp' => 'كود التحقق',
        'full_name' => 'الإسم بالكامل',
        'message' => 'الرسالة',
        'pickup_address' => 'عنوان الإلتقاط',
        'dropoff_address' => 'عنوان النزول',
        'pickup_latitude' => 'خط العرض لعنوان الإلتقاط',
        'pickup_longitude' => 'خط الطول لعنوان الإلتقاط',
        'dropoff_latitude' => 'خط العرض لعنوان النزول',
        'dropoff_longitude' => 'خط الطول لعنوان النزول',
        'date_schedule' => 'تاريخ الحجز',
        'passengers_limit' => 'عدد الركاب',
        'instant_booking' => 'الحجز الفوري',
        'price_per_seat' => 'السعر لكل مقعد',
        'is_publish' => 'نشر الرحلة',
        'middle_seat_empty' => 'المقعد الوسطي فارغ',
        'notes' => 'ملاحظات',
        'ride_id' => 'الرحلة',
        'rating' => 'التقييم',
        'review' => 'التقييم',
        'ride_request_id' => 'طلب الرحلة',
        'status_id' => 'الحالة',
        'driver_id' => 'السائق',
        'user_id' => 'المستخدم',
        'payment_method_id' => 'طريقة الدفع',
        'payment_status_id' => 'حالة الدفع',
        'name' => 'الإسم',
        'description' => 'الوصف',
        'latitude' => 'خط العرض',
        'longitude' => 'خط الطول',
        'address' => 'العنوان',
        'type_id' => 'النوع',
        'language_id' => 'اللغة',
        'is_smoking' => 'التدخين',
        'birthday' => 'تاريخ الميلاد',
        'nationality_id' => 'الجنسية',
        'national_id_card_front_image' => 'صورة البطاقة الأمامية',
        'national_id_card_back_image' => 'صورة البطاقة الخلفية',
        'driving_license_front_image' => 'صورة رخصة القيادة الأمامية',
        'driving_license_back_image' => 'صورة رخصة القيادة الخلفية',
        'license_front_image' => 'صورة الرخصة الأمامية',
        'license_back_image' => 'صورة الرخصة الخلفية',
    ],

];
